    ...
    >
      ...
      <Box
        style={{
          ...
          marginStart: LeftComponent ? spacing.m : undefined,
          marginEnd: RightComponent ? spacing.m : undefined,
        }}
      >
        <HeaderContentWrapper>
          {HeadingComponent ||
            (isHeadingPresent && (
              <Box marginBottom={isFooterPresent || isContentPresent ? "xs" : undefined}>
                <Text
                  variant="heading"
                  text={heading}
                  tx={headingTx}
                  txOptions={headingTxOptions}
                  color={preset === "reversed" ? "background" : undefined}
                  {...HeadingTextProps}
                  style={$headingStyleOverride}
                />
              </Box>
            ))}
          ...
          {ContentComponent ||
            (isContentPresent && (
              <Box
                marginTop={isHeadingPresent ? "xs" : undefined}
                marginBottom={isFooterPresent ? "xs" : undefined}
              >
                <Text
                  variant="body"
                  text={content}
        ...
        {FooterComponent ||
          (isFooterPresent && (
            <Box marginTop={isHeadingPresent || isContentPresent ? "xs" : undefined}>
              <Text
                variant="caption"
                text={footer}
                tx={footerTx}
...
const $alignmentWrapperFlexOptions: Record<
  "top" | "center" | "space-between" | "force-footer-bottom",
...
const $headingPresets: Record<Presets, ThemedStyleArray<TextStyle>> = {
  default: [],
  reversed: [(theme) => ({ color: theme.colors.background })],
}
